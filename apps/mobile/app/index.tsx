import { Status, fr, isNextWeek, isThisWeek, isToday, isTomorrow, useCurrentStatus } from 'core';
...
import { BridgeEvent, useRootContext } from '../app/_layout';
...
export default function ScreenView() {
  const { datas, enableNotifications, loading, onToggleNotifications } = useRootContext();
...
type EventListProps = { events: BridgeEvent[]; title: string; dark: boolean };
const EventList = ({ events, title, dark }: EventListProps) => {
  return events.length ? (
    <View
      ...
      {events.map((data) => (
        <BridgeEventItem dark={dark} key={data.closeAt.getTime()} {...data} />
      ))}
    </View>
  ) : null;
};
